...
from mongoCRUD import MongoConnect
...
if 'users' not in db.listDatabases():
    adminCon = Configs.adminConfident()
    [db.insertDat(col='unique_id', query={"key": col, "id": 0}) for col in ['book', 'user']]
    adminAcc = {"username": adminCon['user'], "password": str(adminCon['pass']), "isdeleted": False,
                "isadmin": True, "id": "user_0", "date": time.time(), "role": "admin",
                "mobile": str(adminCon["mobile"]), "proPic": adminCon["pic"]}
    db.insertDat(col='users', query=adminAcc)
...
global admin
admin = [admin for admin in db.fetchDat(col='users', query={"isdeleted": {'$nin': ["true", True]}, "isadmin": {'$nin': ["false", False]}})]
...
def books(request):
    if request.method == 'POST':
        ...
        if req['key'] == 'bookArrival':
            Books = []
            for book in db.fetchDat(col='books', query={"isdeleted": {'$nin': ["true", True]}}):
        ...
        Books = [books for books in
                           db.fetchDat(col='books', query={"isdeleted": {'$nin': ["true", True]}})]
...
def home(request):
    if request.method == "POST":
        ...
        password = req["pass"]
        userSearch = [user for user in db.fetchDat(col='users', query={"username": userName, "isdeleted": {'$nin': ["true", True]}, "isadmin":True})]
        ...
        if credentials is not False and credentials['username'] == userName and credentials['password'] == password:
            ...
            for user in db.fetchDat(col='users', query={"isdeleted": {'$nin': ["true", True]}, "isadmin": {'$in': ["false", False]}}):
                usersQty += 1
...
def rent(request):
    if request.method == 'POST':
        ...
        if req['key'] != "rentalForm":
            req = req['key'].split("-")
            if req[0] == 'rentBook':
                for book in db.fetchDat(col='books', query={"bookId": req[1]}):
                    options = {"admin": admin, "bookDet": book}
                    return render(request, "admin.rent.html", options)
        else:
            req = eval(req['val'])
            book = list(
                map(lambda x: x, db.fetchDat(col='books', query={"bookId": req["bookId"], "isdeleted": {'$in': ["false", False]}})))
            book = dict(ChainMap(*book)) if len(book) >= 1 else False
            # for book in db.books.find({"bookId": req['bookId'], "isdeleted": {'$in': ["false", False]}}):
            for user in db.fetchDat(col='users', query={"id": req["userName"], "isdeleted": {'$in': ["false", False]}}):
...
def saveUpdateForm(request):
    if request.method == 'POST':
        ...
        if req['key'] == 'addUser':
            ...
            val = req['val']
            for user in db.fetchDat(col='users', query={"id": val, "isdeleted": {'$in': ["false", False]}}):
...
def users(request):
    if request.method == 'POST':
        ...
        users = [user for user in
                           db.fetchDat(col='users', query={"isdeleted": {'$nin': ["true", True]}, "isadmin": {'$in': ["false", False]}})]
        ...
        if request.method == "POST":
            ...
            if req["key"] == "User":
                try:
                    ...
                    idNum = 0
                    for user in db.fetchDat(col='unique_id', query={"key": "user"}):
                        idNum = user['id']+1
                    db.updateDat(col='unique_id', query={"key": 'user'}, data={"id": idNum})
                    ...
                    db.insertDat(col='users', query=userData)
                ...
                try:
                    ...
                    db.updateDat(col='users', query={"id": userId}, data=userData)
                ...
                try:
                    ...
                    image = req["img"] if len(req["img"]) > 1 else noImage()
                    idNum = 0
                    for user in db.fetchDat(col='unique_id', query={"key": "book"}):
                        idNum = user['id']+1
                    db.updateDat(col='unique_id', query={"key": 'book'}, data={"id": idNum})
                    ...
                    db.insertDat(col='books', query=bookData)
                ...
                try:
                    req = req['val']
                    req = eval(req)
                    for rentedBook in db.fetchDat(col='bookRent', query={"userName": req['userName'], "bookId": req['bookId']}):
                        ...
                        book = [x for x in db.fetchDat(col='books', query={"bookId": rentedBook["bookId"], "isdeleted": {'$in': ["false", False]}})]
                        book = dict(ChainMap(*book)) if len(book) >= 1 else False
                        if book:
                            stock = book["stock"]
                            db.updateDat(col='books', query={"bookId": req["bookId"]}, data={"stock": stock+1})
                            db.updateDat(col='bookRent', query={"userName": req["userName"], "bookId": req["bookId"]}, data={"returned": True})
                ...
                try:        # book Rent
                    req = req['val']
                    obj = eval(req)
                    book = [i for i in db.fetchDat(col='books', query={"bookId": obj["bookId"], "isdeleted": {'$in': ["false", False]}})]
                    book = dict(ChainMap(*book)) if len(book) >= 1 else False
                    for user in db.fetchDat(col='users', query={"id": obj["userName"], "isdeleted": {'$in': ["false", False]}}):
                        ...
                        db.insertDat(col='bookRent', query=rentaData)
                        # for book in db.books.find({"bookId": obj["bookId"], "isdeleted": {'$in': ["false", False]}}):
                        book["stock"] -= 1
                        db.updateDat(col='books', query={"bookId": book['bookId']}, data={"stock": book['stock']})
                    ...
                    req = req['val']
                    obj = eval(req)
                    for book in db.fetchDat(col='books', query={"bookId": obj["bookId"]}):
...
def scan(request):
    if request.method == "GET":
        ...
        try:
            ...
            db.bulkWrite(col='books', data=bookColl)
